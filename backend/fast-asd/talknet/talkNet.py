...
class talkNet(nn.Module):
    def __init__(self, lr = 0.0001, lrDecay = 0.95, **kwargs):
        ...
        self.model = talkNetModel().to("mps")
        self.lossAV = lossAV().to("mps")
        self.lossA = lossA().to("mps")
        self.lossV = lossV().to("mps")
    ...
    def train_network(self, loader, epoch, **kwargs):
        ...
        for num, (audioFeature, visualFeature, labels) in enumerate(loader, start=1):
            self.zero_grad()
            audioEmbed = self.model.forward_audio_frontend(audioFeature[0].to("mps")) # feedForward
            visualEmbed = self.model.forward_visual_frontend(visualFeature[0].to("mps"))
            audioEmbed, visualEmbed = self.model.forward_cross_attention(audioEmbed, visualEmbed)
            outsAV= self.model.forward_audio_visual_backend(audioEmbed, visualEmbed)  
            outsA = self.model.forward_audio_backend(audioEmbed)
            outsV = self.model.forward_visual_backend(visualEmbed)
            labels = labels[0].reshape((-1)).to("mps") # Loss
    ...
    def evaluate_network(self, loader, evalCsvSave, evalOrig, **kwargs):
        ...
        for audioFeature, visualFeature, labels in tqdm.tqdm(loader):
            with torch.no_grad():                
                audioEmbed  = self.model.forward_audio_frontend(audioFeature[0].to("mps"))
                visualEmbed = self.model.forward_visual_frontend(visualFeature[0].to("mps"))
                audioEmbed, visualEmbed = self.model.forward_cross_attention(audioEmbed, visualEmbed)
                outsAV= self.model.forward_audio_visual_backend(audioEmbed, visualEmbed)  
                labels = labels[0].reshape((-1)).to("mps")             
    ...
    def loadParameters(self, path):
        ...
        loadedState = torch.load(path, map_location=torch.device('cpu'), weights_only=False)
